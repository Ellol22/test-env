# 📁 courses\management\commands\load_control_sys_courses.py
...
class Command(BaseCommand):
    help = 'Load Control Systems courses from JSON'

    def add_arguments(self, parser):
        parser.add_argument(
            '--file',
            type=str,
            default='data/control_sys_courses.json'
        )
    ...
    def handle(self, *args, **kwargs):
        ...
        control_data = data.get('industrial_control', {})
        ...
        if kwargs['clear']:
            deleted, _ = Course.objects.filter(
                structure__department=DepartmentChoices.CONTROL_SYSTEMS
            ).delete()
        ...
        # 🔁 Loop
        # =========================
        for year_key, year_data in control_data.items():
        ...
        self.stdout.write(self.style.SUCCESS(
            f"✅ Done! Added {count} Control Systems courses"
        ))