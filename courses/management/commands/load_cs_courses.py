# 📁 courses\management\commands\load_cs_courses.py
...
class Command(BaseCommand):
    help = 'Load Cyber Security courses from JSON'

    def add_arguments(self, parser):
        parser.add_argument(
            '--file',
            type=str,
            default='data/cs_courses.json'
        )
    ...
    def handle(self, *args, **kwargs):
        ...
        cs_data = data.get('cyber_department', {})
        ...
        if kwargs['clear']:
            deleted, _ = Course.objects.filter(
                structure__department=DepartmentChoices.CYBER
            ).delete()
        ...
        # 🔁 Loop
        # =========================
        for year_key, year_data in cs_data.items():
        ...
        self.stdout.write(self.style.SUCCESS(
            f"✅ Done! Added {count} Cyber Security courses"
        ))